...
### Auxiliary function to subtract t_bett fom t_spindel and t_motor
...
class AIService:
    ...
    def __extract_time_relevance_array(self, input_array, output_shape):
        ...
        try:
            ...
            while True:
                ...
                if self.__appConfig.make_prediction:
                    signals= self.__preprocessor.execute(np.asarray([new_signal_list]))
                    signals[0]= np.float32(signals[0])
                    if first_run: ###
                        hf_array = np.full(output_sec_shape, signals[0],dtype= np.float32)
                        array = self.__extract_time_relevance_array(hf_array, self.__appConfig.shape)
                    ...
                    elif under_response_time: ##check the trigger
                        #print("....durchshiften1")
                        given2model= False
                        hf_array[0][:-1] = hf_array[0][1:]
                        hf_array[0, -1, :] = signals[0]
                        #continue
                    else:
                        hf_array[0][:-1] = hf_array[0][1:]
                        hf_array[0, -1, :] = signals[0]
                        ...
                        last_prediction_time = current_time
                        given2model = True
                        if self.__appConfig.show_debugprints:
                            print("--> call Model")
                ...
                if self.__appConfig.make_prediction:
                    ...
                    parameter_content = [{"date": current_time,
                                        ...
                                        "preprocessed_data": list(signals[0]),
